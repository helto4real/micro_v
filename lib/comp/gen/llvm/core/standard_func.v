...
fn (mut m Module) add_exit() {
	...
	// puts function
	exit_function_type := C.LLVMFunctionType(C.LLVMVoidTypeInContext(m.ctx_ref), exit_function_args_type.data, 1, false)
	// Add puts
...
fn (mut m Module) add_longjmp_setjmp() {
	...
	mut value_refs := [C.LLVMConstInt(get_llvm_type_ref(symbols.i64_symbol, m),
						i64(0), false)]

	res := C.LLVMConstNamedStruct(jmp_buf_typ_ref, value_refs.data,
                                    value_refs.len)
	...
	// puts function
	longjmp_function_type := C.LLVMFunctionType(C.LLVMVoidTypeInContext(m.ctx_ref), longjmp_function_args_type.data, 2, false)
	// Add puts
	...
	// puts function
	setjmp_function_type := C.LLVMFunctionType(C.LLVMInt64TypeInContext(m.ctx_ref), setjmp_function_args_type.data, 1, false)
	// Add puts
