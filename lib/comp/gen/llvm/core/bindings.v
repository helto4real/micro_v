...
pub fn (c Use) str() string {
	return 'Use ($c.c)'
}

pub fn (l Type) isnil() bool {
	return l.c == 0
}

pub fn (l Value) isnil() bool {
	return l.c == 0
}

pub fn (l BasicBlock) isnil() bool {
	return l.c == 0
}

pub fn (l Builder) isnil() bool {
	return l.c == 0
}

pub fn (l PassManager) isnil() bool {
	return l.c == 0
}
...
pub fn (l Type) type_ref() &C.LLVMTypeRef {
	return l.c
}

pub fn (l Value) value_ref() &C.LLVMValueRef {
	return l.c
}
...
pub fn (t Type) type_kind() LLVMTypeKind {
	return C.LLVMGetTypeKind(t.c)
}
...
// context
pub fn (c Context) int1_type() Type {
	return Type{
		c: C.LLVMInt1TypeInContext(c.c)
	}
}

pub fn (c Context) int8_type() Type {
	return Type{
		c: C.LLVMInt8TypeInContext(c.c)
	}
}

pub fn (c Context) int16_type() Type {
	return Type{
		c: C.LLVMInt16TypeInContext(c.c)
	}
}

pub fn (c Context) int32_type() Type {
	return Type{
		c: C.LLVMInt32TypeInContext(c.c)
	}
}

pub fn (c Context) int64_type() Type {
	return Type{
		c: C.LLVMInt64TypeInContext(c.c)
	}
}
...
pub fn (c Context) void_type() Type {
	return Type{
		c: C.LLVMVoidTypeInContext(c.c)
	}
}
...
pub fn new_function_type(ret_type Type, param_types []Type, is_variadic bool) Type {
	...
	ft := C.LLVMFunctionType(ret_type.c, c_param_types.data, c_param_types.len, bool_to_llvm_bool(is_variadic))
...
pub fn (t Type) create_const_array(const_values []Value) Value {
	...
	return Value{
		c: C.LLVMConstArray(t.c, c_values.data, c_values.len)
	}
}
...
pub fn (t Type) create_const_named_struct(const_values []Value) Value {
	c_values := value_refs(const_values)
	return Value{
		c: C.LLVMConstNamedStruct(t.c, c_values.data, c_values.len)
	}
}
...
pub fn (t Type) is_fn_var_arg() bool {
	return C.LLVMIsFunctionVarArg(t.c) != C.LLVMBool(0)
}

pub fn (t Type) return_type() Type {
	return Type{
		c: C.LLVMGetReturnType(t.c)
	}
}

pub fn (t Type) params_typ_count() int {
	return C.LLVMCountParamTypes(t.c)
}

pub fn (t Type) const_ptr_null() Value {
	return Value{
		c: C.LLVMConstPointerNull(t.c)
	}
}

pub fn (t Type) const_null() Value {
	return Value{
		c: C.LLVMConstNull(t.c)
	}
}

pub fn (t Type) dump_type() {
	C.LLVMDumpType(t.c)
}
...
pub fn (c Context) new_struct_type(element_types []Type, packed bool) Type {
	...
	struct_typ := C.LLVMStructTypeInContext(c.c, types.data, types.len, bool_to_llvm_bool(packed))
...
pub fn (bb BasicBlock) as_value() Value {
	return Value{
		c: C.LLVMBasicBlockAsValue(bb.c)
	}
}

// values
pub fn (v Value) value_kind() LLVMValueKind {
	return C.LLVMGetValueKind(v.c)
}

pub fn (v Value) typ() Type {
	return Type{
		c: C.LLVMTypeOf(v.c)
	}
}

pub fn (v Value) dump_value() {
	C.LLVMDumpValue(v.c)
}

pub fn (v Value) is_a_argument() Value {
	return Value{
		c: C.LLVMIsAArgument(v.c)
	}
}

pub fn (v Value) is_a_terminator_instruction() Value {
	return Value{
		c: C.LLVMIsATerminatorInst(v.c)
	}
}

pub fn (v Value) is_constant() bool {
	return llvm_bool_to_bool(C.LLVMIsConstant(v.c))
}

pub fn (v Value) is_null() bool {
	return llvm_bool_to_bool(C.LLVMIsNull(v.c))
}

pub fn (v Value) set_initializer(val Value) {
	C.LLVMSetInitializer(v.c, val.c)
}

pub fn (v Value) param(index int) Value {
	return Value{
		c: C.LLVMGetParam(v.c, index)
	}
}

pub fn (v Value) first_use() Use {
	return Use{
		c: C.LLVMGetFirstUse(v.c)
	}
}

pub fn (u Use) next_use() Use {
	return Use{
		c: C.LLVMGetNextUse(u.c)
	}
}
...
pub fn const_int(t Type, val u64, sign_extend bool) Value {
	return Value{
		c: C.LLVMConstInt(t.c, val, bool_to_llvm_bool(sign_extend))
	}
}

pub fn (c Context) c_i1(val int, sign_extend bool) Value {
	return const_int(c.int1_type(), u64(val), sign_extend)
}

pub fn (c Context) c_i8(val int, sign_extend bool) Value {
	return const_int(c.int8_type(), u64(val), sign_extend)
}

pub fn (c Context) c_i16(val int, sign_extend bool) Value {
	return const_int(c.int16_type(), u64(val), sign_extend)
}

pub fn (c Context) c_i32(val int, sign_extend bool) Value {
	return const_int(c.int32_type(), u64(val), sign_extend)
}

pub fn (c Context) c_i64(val u64, sign_extend bool) Value {
	return const_int(c.int64_type(), val, sign_extend)
}
...
pub fn (b Builder) create_call2(fn_typ Type, fn_val Value, args []Value) Value {
	return Value{
		c: C.LLVMBuildCall2(b.c, fn_typ.c, fn_val.c, args.data, args.len, no_name.str)
	}
}
...
pub fn (b Builder) create_gep2(typ Type, val Value, indicies []Value) Value {
	return Value{
		c: C.LLVMBuildInBoundsGEP2(b.c, typ.c, val.c, indicies.data, indicies.len, no_name.str)
	}
}
